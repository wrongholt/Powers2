...
BasicGame.Boot.prototype = {
  ...
  scaleStage: function () {
    ...
    if (this.scale.scaleMode == Phaser.ScaleManager.NO_BORDER) {
      BasicGame.viewX =
        (this.scale.width / 2 - window.innerWidth / 2) *
        this.scale.scaleFactor.x;
      BasicGame.viewY =
        (this.scale.height / 2 - window.innerHeight / 2 - 1) *
        this.scale.scaleFactor.y;
    ...
    document.getElementById('game').style.height =
      window.innerHeight - 1 + 'px'; //The css for body includes 1px top margin, I believe this is the cause for this -1
